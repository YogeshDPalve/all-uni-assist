...
import { AI_API } from "../../constant/constant";
import axios from "axios";

function AiInput({ value, onChange, onSubmit, onKeyDown }) {
      ...
      <div className="relative mx-auto flex w-full max-w-4xl flex-col items-start gap-2">
        <div className="relative mx-auto w-full max-w-4xl">
          ...
          <button
            ...
            disabled={!value.trim()}
          >
            <CornerRightUp className="text-primary h-4 w-4" />
          </button>
        </div>
...
export default function WorkingChatbot() {
  ...
  const fetchAIResponse = async (userInput) => {
    try {
      ...
      setResponseTimes((prev) => ({ ...prev, [aiMessage.id]: duration }));
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="mx-auto flex h-svh w-full max-w-4xl flex-col p-1">
